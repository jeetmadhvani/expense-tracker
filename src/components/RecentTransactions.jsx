...
const RecentTransactions = ({transactions, onDelete, onAddClick}) => {
  const lastFour = transactions.slice(0, 4);
  return (
    <div className='bg-[#222222] w-full h-full rounded-3xl p-5 text-[#eaeaea] shadow-[0_0_20px_rgba(0,0,0,0.3)] hover:shadow-[0_0_25px_rgba(0,0,0,0.5)] transition-all border border-white/5 bg-linear-to-br from-[#222] to-[#1d1d1d]'>
        ...
        <div className='h-85 w-full mt-10 flex flex-col gap-2'>
          {lastFour.length === 0 && (
          <p className="text-[#a0a0a0] text-3xl h-full flex justify-center items-center pb-15">No transactions yet.</p>
      )}
      ...
      {lastFour.map((t, idx) => (
        <TransactionItem
          ...
          idx={idx + 1}
          title={t.title}
          amt={t.amount.toLocaleString("en-IN")}
          category={t.category}
          date={t.date}
          newItem={t.justAdded}
          desc={t.description}
